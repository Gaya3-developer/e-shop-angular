...
router.get("/", async (req, res) => {
  ...
  db.query(
    `SELECT p.id, p.title, p.image, p.price, p.short_desc, p.quantity FROM products p`,
    (err, results) => {
      if (err) console.log(err);
      else res.json(results);
    }
...
router.get("/:productId", async (req, res) => {
  ...
  db.query(
    `SELECT p.id, p.title, p.image, p.images, p.description, p.price, p.quantity, p.short_desc,
        c.title as category, c.id as cat_id FROM products p JOIN categories c ON
...
router.get("/categories/:categoryId", async (req, res) => {
  ...
  db.query(
    `SELECT p.id, p.title, p.image, p.images, p.description, p.price, p.quantity, p.short_desc
        FROM products p  WHERE p.cat_id = ${categoryId}`,
    (err, results) => {
      if (err) console.log(err);
      else res.json(results);
    }
